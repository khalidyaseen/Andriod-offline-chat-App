...
public class SelectorAsyncTask extends AsyncTask<Void, Void, Void>{
	...
	public void doReadable(SocketChannel schannel){
		...
		if( data != null ){
            Log.d("ERROR"," string len  "+data.length());
			Bundle b = new Bundle();
			b.putString("DATA", data);
            Log.d("ERROR","BUNDLED String "+b.toString().length());
	...
	public String readData(SocketChannel sChannel) {
        ...
        Log.d("ERROR","inside read data");
		ByteBuffer buf = ByteBuffer.allocate(1024*1024);   // let's cap json string to 4k for now.
		byte[] bytes = null;
    ...
    //    bytes = new byte[1024 * 1024 * 10];
		...
		try {
		    ...
		    numBytesRead = sChannel.read(buf);
               ...
               Log.d("ERROR","reading bytes "+numBytesRead);
               if (numBytesRead == -1) {
                   ...
                   Log.d("ERROR", "bufLimit " + buf.limit() + " POSTision " + buf.position() + " :LIMIT " + buf.limit() + " : " + buf.capacity());
       ...
       //            buf.get(bytes);
           ...
           //        count++;
                  ...
                  // while ( buf.hasRemaining() ) {
                       buf.get(bytes);
                   //}
                   //  Log.d("ERROR", "bytesRead" +bytes.toString());
                   //}
                   //Log.d("ERROR","bytes.length" + bytes.length);
                   Log.d("ERROR","bytearray "+bytes.length);
               }


		    jsonString = new String(bytes);  // convert byte[] back to string.
            Log.d("ERROR","inside the reading after reading");
            Log.d("ERROR","Strign size "+jsonString.length());

		}catch(Exception e){
			Log.e(TAG, "readData : exception: " + e.toString());
            Log.d("ERROR","readding  " + e.toString());

			notifyConnectionService(MSG_BROKEN_CONN, sChannel, null);
		}
		
		Log.d(TAG, "readData: content: " + jsonString);
        Log.d("ERROR","readContent: "+jsonString);
		return jsonString; 
	}
	...
	private void notifyConnectionService(int what, Object obj, Bundle data){
        ...
        Log.d("ERROR","inside notification");
		hdl.sendMessage(msg);		
	}
